...
describe('recording', () => {
  it('keeps both sides of the conversation in the order they were said', () => {
    ...
    history.append({ role: 'user', content: 'there is a door with no handle' });
    history.append({ role: 'assistant', content: 'Roll me. Twelve. It opens, and it was never locked.' });

    const [conversation] = history.conversations;
    assert.deepEqual(conversation.messages.map((m) => [m.role, m.content]), [
      ['user', 'there is a door with no handle'],
      ['assistant', 'Roll me. Twelve. It opens, and it was never locked.'],
    ]);
...
describe('picking one back up', () => {
  function stored() {
    ...
    history.begin({ voice: 'leo' });
    history.append({ role: 'user', content: 'what is behind the door?' });
    history.append({ role: 'assistant', content: 'Roll me. Eleven. It opens.' });
  ...
  it('hands back what was said in the one it reopens', () => {
    ...
    assert.deepEqual(earlier.messages.map((m) => m.content), ['what is behind the door?', 'Roll me. Eleven. It opens.']);
  });
