...
`SSL_CERT`. Build it yourself with `docker build -t nat .`. Publishing from a
...
The log is a record. Memory is what Nat carries into the next call: a short list
of details, kept in `localStorage` under `nat.memory.v1` and appended to the
persona as a labelled block when the call opens.
...
the page against browser storage, and the result goes back up as a
