 ...
 * vocabulary. The page wires those events to Nat once; Nat
 ...
 *   'level'        0..1 sustained amplitude — mic while listening, Nat while speaking
 ...
 * page's wiring block and Nat do not change.
 */
...
const MIC_CONSTRAINTS = {
  audio: {
    // Nat's voice comes out of the speakers and straight back into the mic.
...
export function createVoiceSession({ model, voice } = {}) {
  ...
  /* One loop for both jobs that have to happen every frame: metering, and
     ...
     that knows when Nat has actually stopped talking — `response.done` fires
  ...
  return {
     ...
     *  stop; cancelling one without the other leaves Nat talking into a turn
