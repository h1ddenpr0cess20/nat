...
under `nat.memory.v1`. Neither is uploaded — the proxy holds no copy of either. The tool switches are a third,
`nat.tools.v1`, holding the names that are switched *off* — so a tool nobody
